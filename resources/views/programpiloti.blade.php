...
<script>
 var time = new Date();
  ...
  <div class="modal-wrapper hidden">
    ...
    @if(Session::get('user')->tip_angajat !== 'Director piloti')
      <div class="row">
  ...
  <div class="hidden modal-row-blueprint">
    <div class="row">
      ...
      <!-- @if(Session::get('user')->tip_angajat == 'Director piloti')
      <input class="activity" value="tip_activitate">
      @endif -->
      
      <select class="activity" {{Session::get('user')->tip_angajat == 'Director piloti' ?"disabled":""}}>
  ...
  <script>
    ...
    */
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.scs) {
            ...
            data.avpiloti.forEach(pilot =>{
               var option = document.createElement('option');
               option.value = pilot[0]
               option.innerText = pilot[1];
    ...
    $('.save').on('click',function(ev){
      ...
      var piloti = []
      var piloti_old = $('.modal .pilots-list .row').toArray();
      piloti_old.forEach(pilot=>{
         if($(pilot).find('.activity').val() == '0')
              return;
         else{
           temp = {};
           
           temp.idAngajat = $(pilot).attr("aid");
           temp.tip_activitate = $(pilot).find('.activity').val();
           piloti.push(temp);
         }
      })
    

      payload["piloti"]=piloti;
      console.log(payload);
      var piloti_new = []
      var piloti_ = $('.modal .new-rows .row').toArray();
      piloti_.forEach(pilot=>{
         if($(pilot).find('.pilots').val() == '-1')
              return;
         else{
           temp = {};
           temp.idAngajat = $(pilot).find('.pilots').val();
           piloti_new.push(temp);
         }
      })

      payload['piloti_new']=piloti_new;
      console.log(payload);
      payload['timestamp']= $('.modal').attr("data");
      $.ajax({
        url:'/saveProgramPiloti',
        type:"POST",
        data:payload,
        success:function(data){
          console.log("SCS!!!!");
    ...
    function arataModal(date,timestamp) {
      date.forEach(function (line) {
         ...
         $('.pilots-list')[0].appendChild(empty_row[0]);
